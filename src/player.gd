...
func _ready():
	...
	# Initialize double jump if already unlocked
	if Global.has_ability("double_jump"):
		can_double_jump = true
		has_double_jump = true
	# Add player to group for UI access
	add_to_group("player")
...
func _physics_process(delta):
	...
	# Dash (LShift)
	if Input.is_action_just_pressed("ui_dash") and not is_dashing and Global.has_ability("dash"):
		is_dashing = true
	...
	if Input.is_action_pressed("ui_attack"):
		if not charging:
			...
			charge_timer += delta
	elif Input.is_action_just_released("ui_attack") and charging:
		perform_attack(charge_timer)
		charging = false
		charge_timer = 0.0

	# Debug: change essence with E
	if Input.is_action_just_pressed("ui_essence"):
		cycle_essence()

	# Godot 4: move_and_slide() is called without Vector2.UP
...
func get_wall_direction() -> int:
	...
	if is_on_wall():
		...
		var left_hit = space.intersect_point(left_point)
		var right_hit = space.intersect_point(right_point)
		if left_hit.size() > 0:
			return -1
		if right_hit.size() > 0:
			return 1
